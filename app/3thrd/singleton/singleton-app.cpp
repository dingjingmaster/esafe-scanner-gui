        ...
        : QApplication (argc, argv)
{
    ...
    if(ret) {
        ...
        if (!mMemory->attach()) {
            qCritical() << "SingletonApplication: Unable to attach to shared memory block. error:" << mMemory->errorString();
            ::exit( EXIT_FAILURE );
        }
    }
...
quint32 SingletonApp::instanceId()
{
    return mInstanceNumber;
}

bool SingletonApp::sendMessage(QByteArray message, int timeout)
{
    if(isPrimary()) {
