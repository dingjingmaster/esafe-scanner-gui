...
public:
     ...
     * @note 超时只是对阻塞操作的最大时间的一个提示。它不能保证SingletonApplicatio初始化将在给定的时间内完成
     */
    SingletonApp (int &argc, char *argv[], const char *appName = "SingleApplication", bool allowSecondary = false, Options options = Mode::User, int timeout = 1000);
    ~SingletonApp ();
...
Q_DECLARE_OPERATORS_FOR_FLAGS(SingletonApp::Options)
